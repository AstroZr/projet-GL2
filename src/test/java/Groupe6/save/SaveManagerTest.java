 ...
 *
 * <p>Teste les fonctionnalités critiques: - Gestion des profils joueurs (création, chargement,
 * sauvegarde) - Gestion des parties en cours (sauvegarde et chargement) - Gestion des meilleurs
 * temps - Chargement des niveaux
 */
public class SaveManagerTest {
  ...
  public void testCreerNouveauJoueur() {
    ...
    assertFalse(
        "Le joueur de test ne doit pas exister au départ", joueursAvant.contains(TEST_PLAYER_NAME));
    ...
    assertTrue(
        "Le joueur doit être créé après la sauvegarde", joueursApres.contains(TEST_PLAYER_NAME));
  }
  ...
  public void testSauvegarderEtChargerParametres() {
    ...
    assertEquals(
        "Le nom joueur doit correspondre", TEST_PLAYER_NAME, parametresCharges.getNomJoueur());
  ...
  public void testSauvegarderEtChargerPartie() {
    ...
    SaveManager.sauvegarderPartie(TEST_PLAYER_NAME, "exemple_reste", partie);

    // Charger la partie
    PartieSauvegardee partieChargee = SaveManager.chargerPartie(TEST_PLAYER_NAME, "exemple_reste");
  ...
  public void testEnregistrerMeilleurTemps() {
    ...
    SaveManager.enregistrerMeilleurTemps(TEST_PLAYER_NAME, "Facile 1", temps1);
    ...
    assertTrue("Le niveau Facile 1 doit être dans le map", temps.containsKey("Facile 1"));
    assertEquals("Le meilleur temps doit correspondre", temps1, (long) temps.get("Facile 1"));
  }
  ...
  public void testMettreAJourMeilleurTemps() {
    ...
    SaveManager.enregistrerMeilleurTemps(TEST_PLAYER_NAME, "Moyen 1", 120000);

    // Enregistrer un temps meilleur
    SaveManager.enregistrerMeilleurTemps(TEST_PLAYER_NAME, "Moyen 1", 90000);

    // Vérifier que le meilleur temps a été mis à jour
    Map<String, Long> temps = SaveManager.chargerMeilleursTemps(TEST_PLAYER_NAME);
    assertEquals("Le meilleur temps doit être mis à jour", 90000, (long) temps.get("Moyen 1"));
  }
  ...
  public void testNePasMetAJourSiTempsPlusMauvais() {
    ...
    Map<String, Long> temps = SaveManager.chargerMeilleursTemps(TEST_PLAYER_NAME);
    assertEquals(
        "Le meilleur temps ne doit pas changer si le nouveau est plus mauvais",
        90000,
        (long) temps.get("Moyen 1"));
  }
  ...
  public void testChargerNiveauExistant() {
    Niveau niveau = SaveManager.chargerNiveau("Facile 1");

    assertNotNull("Le niveau 'test' doit être chargeable", niveau);
    assertEquals("L'ID du niveau doit correspondre", "Facile1", niveau.getId());
