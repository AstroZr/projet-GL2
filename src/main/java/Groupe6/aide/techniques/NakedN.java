...
public class NakedN extends AideAbstract {
    ...
    @Override
    public boolean isOverUsed() { return this.nbUtilisation >= 3; }
    ...
    private Set<Integer> getCandidats(Cellule cellule, Grille grille) {
        ...
        for (int v = 1; v <= taille; v++)
            if (!dejaPresents.contains(v)) candidats.add(v);
    ...
    private Set<Integer> getValeursPossiblesPourCellule(ZoneCalcul zone, Cellule cellule, int taille) {
        ...
        for (List<Integer> combo : zone.trouverCombinaisons(taille))
            if (index < combo.size()) possibles.add(combo.get(index));
        return possibles;
    }

    /**
     * Génère toutes les combinaisons de taille `taille` depuis une liste de cellules.
    ...
    public int load(Grille grille, int nbAides) {
        ...
        for (Cellule c : this.groupe){
            System.out.println("\tline : " + c.getLigne() + " column : " + c.getColonne());
        }
        ...
        */
        ...
        switch (this.nbUtilisation) {
            ...
            case 1:
                this.aideTextuel = new AideTextuel(this.titre, this.description +
                    " Les candidats sont : " + String.valueOf(this.unionCandidats));
                break;
            ...
            default:
                ...
                for (Cellule cellule : this.groupe){
                    ...
                    for (int numero : this.unionCandidats){
                        if (!candidats.contains(numero)){
                            ...
                            this.aideVisuel.add(new EffetVisuel(TypeEffect.CANDIDAT_POSITIF, cellule.getLigne(), cellule.getColonne(), String.valueOf(numero)));
                        }
                    }
                }
                ...
                for (Cellule cellule : this.impact){
                    ...
                    for (int numero : this.unionCandidats){
                        if (candidats.contains(numero)){
                            this.aideVisuel.add(new EffetVisuel(TypeEffect.CANDIDAT_NEGATIF, cellule.getLigne(), cellule.getColonne(), String.valueOf(numero)));
                        }
                    }
                }
        }
