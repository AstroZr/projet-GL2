...
/**
 * Coeur du jeu : boucle update/render découplée (UPS fixe, FPS limité), délégation aux états
 * (Start, Menu, etc.). Utilise un pattern State Machine pour gérer les différents écrans (Menu,
 * Jeu, Paramètres, etc.).
 *
 * <p>Architecture: - Une EnumMap mappe EtatJeu -> MethodesEtats pour accès O(1) aux états - Boucle
 * asynchrone avec accumulation de temps pour UPS stable - Rendu découplé (FPS limité) indépendant
 * des updates - Gestion des transitions fluides entre états avec fade-out/fade-in
 */
public class Game implements Runnable {
  ...
  private void initClasses() {
    ...
    tutoriel = new Tutoriel(this);
    stateByEnum.put(EtatJeu.TUTORIEL, tutoriel);
  ...
  private void notifierChangementLangue() {
    ...
    if (tutoriel != null) tutoriel.updateTexts();
    if (astuces != null) astuces.updateTexts();
  }
